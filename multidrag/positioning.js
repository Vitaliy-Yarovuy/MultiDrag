		...
		switch(type){
			...
			case positionType.floatLeft:
				return function(rectangle, options){
					...
					return function(rectangleList, indexesOfNews){
						var boundRect = typeof rectangle === "function" ? rectangle() : rectangle, rectP2 = boundRect.getRightTopPoint(), i, boundaryPoints = [boundRect.position];
						rectangleList.forEach(function(rect, index){
							...
							for(i = 0; i < boundaryPoints.length; i++){
								...
								if(isValid = (position.x + rect.delta.x < rectP2.x)){
									break;
								}
							}
							if(!isValid){
							...
							if(opts.isRemove && rect.getRightBottomPoint().y > boundRect.getRightBottomPoint().y){
								rect.isRemove = true;
							}
							boundaryPoints = mathPoint.addPointToBoundPoints(boundaryPoints, rect.getRightBottomPoint().add(opts.paddingBottomRight));
						});
						return rectangleList;
					};
				};
				...
				return function(rectangle, options){
					...
					return function(rectangleList, indexesOfNews){
						...
						rectangleList.forEach(function(rect, index){
							var position, isValid = false;
							for(i = 0; i < boundaryPoints.length; i++){
								position = (new Point(boundaryPoints[i].x - rect.delta.x, i > 0 ? boundaryPoints[i - 1].y : boundRect.position.y)).add(paddingTopNegRight);
								if(isValid = (position.x > rect.position.x)){
									break;
								}
							}
							if(!isValid){
								position = new Point(boundRect.getRightTopPoint().x, boundaryPoints[boundaryPoints.length - 1].y);
							}
							rect.position = position;
							if(opts.isRemove && rect.getLeftBottomPoint().y > boundRect.getLeftBottomPoint().y){
								rect.isRemove = true;
							}
							boundaryPoints = mathPoint.addPointToBoundPoints(boundaryPoints, rect.getLeftBottomPoint().add(paddingBottomNegLeft), true);
						});
						return rectangleList;
					};
				};
