(function(global, MultiDrag){
	'use strict';
	var spiders = [], Spider = function(area, elements, options){
	...
	Spider.prototype.init = function(elements){
		...
		this.objs = elements.map(function(element, i){
			var angle = this.options.angle + i * this.options.dAngle, halfSize = mathPoint.getSizeOfElement(element).mult(0.5), start = mathPoint.getPointFromRadialSystem(angle, this.options.startRadius, this.options.center).sub(halfSize), end = mathPoint.getPointFromRadialSystem(angle, this.options.endRadius, this.options.center).sub(halfSize), bound = function(){
	...
	Spider.prototype.createCanvas = function(){
		...
		this.canvas.setAttribute("width", this.areaRectangle.delta.x + "px");
		this.canvas.setAttribute("height", this.areaRectangle.delta.y + "px");
		MultiDrag.util.setStyle(this.canvas, {
			position: "absolute",
			top: "0",
			left: "0",
			width: this.areaRectangle.delta.x + "px",
			height: this.areaRectangle.delta.y + "px"
		});
	...
	Spider.prototype.draw = function(){
		...
		this.context.clearRect(0, 0, this.areaRectangle.delta.x, this.areaRectangle.delta.y);
