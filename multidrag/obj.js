(function(global, MultiDrag){
	...
	var isTouch = 'ontouchstart' in window, mouseEvents = {
		...
		objs.push(obj);
			...
			Obj = function(element, options){
				options = options || {};
				var i, that = this, displayListener, parent = options.parent || MultiDrag.util.getDafaultParent(element);
				this.options = {
					parent: parent,
					bound: function(){
	...
	Obj.prototype.init = function(){
		...
		this.element.addEventListener(events.start, MultiDrag.util.bind(this.multiDragStart, this));
	...
	Obj.prototype.getSize = function(recalulate){
		if(!this._size || this.options.isReCalculateSize || recalulate){
			this._size = mathPoint.getSizeOfElement(this.element, this.options.isContentBoxSize);
		}
		return this._size;
	};
	...
	Obj.prototype._setTranslate = function(point){
		...
		var webkitTransform = this.element.style.webkitTransform, webkitTranslateCss = 'translate3d(' + point.x + 'px,' + point.y + 'px,0px)';
		...
		this.element.style.webkitTransform = webkitTransform;
	};
	...
	Obj.prototype.move = function(point, time, isFix, isSilent){
		...
		this.position = point;
		this.element.style.webkitTransitionDuration = time + "ms";
	...
	Obj.prototype.multiDragStart = function(event){
		...
		MultiDrag.util.addClass(this.element, "active");
		this.onMove.fire();
	};
	...
	Obj.prototype.multiDragEnd = function(event){
		...
		MultiDrag.util.removeClass(this.element, "active");
	};

	Obj.prototype.changeHtmlParent = function(element){
		if(!this._initialParent){
			this._initialParent = this.element.parentNode;
		}
		...
		MultiDrag.util.waitCall(function(){
			this.offset = mathPoint.getOffset(this.element, this.options.parent, true).sub(this._transformPosition);
	...
	Obj.prototype.refresh = function(){
		...
		//this.offset = mathPoint.getOffset(this.element, this.options.parent, true).sub(this._transformPosition || new Point(0,0) );
		//this.move(this.position,0,false,true);
	}
